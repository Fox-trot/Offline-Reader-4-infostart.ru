...
Процедура ЛогоНажатие(Элемент)
	...
	НаКлиенте.ОткрытьПоСсылке(АдресИнформацииОКонфигурации());
КонецПроцедуры
...
Процедура ГрафикОбработкаРасшифровки(Элемент, Расшифровка, СтандартнаяОбработка)
	Если ТипЗнч(Расшифровка) = Тип("Булево") Тогда
		...
		тФорма = ПолучитьФорму("Документ.Публикация.ФормаСписка", Новый Структура("КлючНазначенияИспользования,Отбор", Строка(Новый УникальныйИдентификатор), Новый Структура("Проведен,ПометкаУдаления", Ложь, Ложь)));
		тФорма.Открыть();
	КонецЕсли;
КонецПроцедуры

&НаСервере
Функция АдресИнформацииОКонфигурации()
	Возврат Инфостарт.АдресИнформацииОКонфигурации();
КонецФункции
