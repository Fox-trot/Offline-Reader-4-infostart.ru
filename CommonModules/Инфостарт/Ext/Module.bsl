...
Функция Цензура(Знач Слово)
	...
	Слова.Вставить("&quot;",	"""");
	//Слова.Вставить("&amp;",		"&");
	Для Каждого ТекСтрока Из Слова Цикл
		Слово = СтрЗаменить(Слово, ТекСтрока.Ключ, ТекСтрока.Значение);
	КонецЦикла;
	Возврат Слово;
КонецФункции
...
Функция ЭтоЧисло(Знач Нумер, БольлшеНоля=Ложь)
	//Если ПустаяСтрока(Нумер) И НЕ БольлшеНоля Тогда Возврат Истина; КонецЕсли;
	Попытка
		Пробка = Число(Нумер);
	Исключение
		Возврат Ложь;
...
Функция Парсить(Текст, Канал)
	...
	ЭтоИнфостарт	= (Канал.Парсер = Перечисления.Парсеры.Инфостарт);
	Запрос			= Новый Запрос(
		"ВЫБРАТЬ
					...
					Пока ЧтениеXML.Прочитать() Цикл
						...
						ИначеЕсли ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
							Если ИмяУзла = "title" Тогда
								ФрешСтеп.Заголовок	= Цензура(ЧтениеXML.Значение);
	...
	Если НЕ ПустаяСтрока(Гиперссылка) Тогда
		Попытка
			...
			Если СтруктураURI.Схема = "https" Тогда
				HTTPСоединение	= Новый HTTPСоединение(СтруктураURI.Хост, СтруктураURI.Порт,,,, ТаймАут, Новый ЗащищенноеСоединениеOpenSSL);
			Иначе
...
Функция СтруктураURI(Знач Гиперссылка) Экспорт
	...
	Если Позиция > 0 Тогда
		...
		Если Не ЭтоЧисло(Порт, Истина) Тогда
			Порт = "";
		КонецЕсли;
	КонецЕсли;
