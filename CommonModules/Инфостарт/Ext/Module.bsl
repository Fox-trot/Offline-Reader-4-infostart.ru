...
Функция ДатаПреобразовать(Знач Слово)
	...
	Если СтрНайти(Слово, "+") > 0 Тогда
		Попытка
			Сдвиг = Число(Сред(Слово, СтрНайти(Слово, "+") + 1));
			Слово = Лев(Слово, СтрНайти(Слово, "+") - 1);
		Исключение
			Сдвиг = 0;
		КонецПопытки;
	...
	КонецЕсли;
	День	= "";
	Месяц	= Формат(Месяц(ТекущаяДатаСеанса()), "ЧЦ=2; ЧВН=");
	Год		= Формат(Год(ТекущаяДатаСеанса()), "ЧЦ=4; ЧВН=");
	Время	= "";
	...
	Если НЕ ПустаяСтрока(День) Тогда
		Слово = СокрЛП(Сред(Слово, СтрНайти(Слово, " ")));
		...
		Для Каждого тЭлемент Из Кондуит Цикл
			Если СтрНайти(Слово, тЭлемент) > 0 Тогда
				Месяц	= Формат(Кондуит.Найти(тЭлемент) + 1, "ЧЦ=2; ЧВН=");
				Год		= Лев(СокрЛП(Сред(Слово, СтрНайти(Слово, тЭлемент) + 4)), 4);
				Если СтрНайти(Слово, ":") > 0 Тогда
					Время	= СокрЛП(СтрЗаменить(Сред(Слово, СтрНайти(Слово, ":") - 2), ":", ""));
				КонецЕсли;
				Прервать;
			КонецЕсли;
		КонецЦикла;
...
Функция АвторНайти(Канал, Знач Слово="")
	...
	Код = "";
	Если НЕ ПустаяСтрока(Слово) И СтрНайти(Слово, "infostart.ru/profile") > 0 Тогда
		Слово = Сред(Слово, СтрНайти(Слово, "infostart.ru/profile/") + 21);
		Для Итератор=1 По СтрДлина(Слово) Цикл
	...
	Иначе
		...
		Если Автор.Пустая() Тогда
			...
			Наименование = Лев(Наименование, СтрНайти(Наименование, "<") - 1);
...
Функция Парсить(Текст, Канал, КомПарсер=Неопределено, КомЗагрузчик=Неопределено)
	...
	Для Каждого Элемент ИЗ СписокЭлементов Цикл
		...
		Если Канал.Парсер = Перечисления.Парсеры.Инфостарт Тогда
			...
			Если ЭтоЧисло(ФрешСтеп.Номер) Тогда
	...
	Если Регламент.ВестиПротоколЗагрузки() Тогда
		...
		//Запись.Канал		= Канал;
		НЗ = РегистрыСведений.Загрузки.СоздатьНаборЗаписей();
		НЗ.Отбор.Канал.Установить(Канал);
		НЗ.Прочитать();
...
Функция ЗагрузитьRSS(Знач Канал=Неопределено) Экспорт
	...
	Запрос.УстановитьПараметр("Предел",	ТекущаяДатаСеанса() - 180);				// минимальный период между обновлениями
	Если ПустаяСтрока(Канал) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Канал = &Канал", "Канал.ПометкаУдаления = ЛОЖЬ");
	КонецЕсли;
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Если ТекущаяДатаСеанса() - Выборка.Период > Выборка.ПериодОбновления Тогда
